    ...
    <div className="D_CTW_D_trendContainer">
      <div className="D_CTW_D_trendContainerInner">
        {value && <div>
          ...
          {date && <span className="D_CTW_D_trendDate">since {dateFormats.monthYear(date)}</span>}
        </div>}
      </div>
    </div>
  )
  ...
  return (
    <article className="D_CTW_D_root" role="article">
      <span className="D_CTW_D_tooltipContainer">
        {tooltipAnchorTo ? <Tooltip anchorTo={tooltipAnchorTo} iconOnly={true} text="" /> : <span>&nbsp;</span>}
      </span>
